...
exports.handler = async (event) => {
    ...
    for (const raw of rawRows) {
        ...
        if (!contracts.has(id)) {
            contracts.set(id, { ...row, transactions: [] });
        }
...
/* ========================= UPDATED PROCESS CONTRACT ========================= */
async function processContract(id, contract, existingItem) {
    // ✅ 1. Check for closed values and return status
    ...
    const enriched = sorted.map((t) => {
        ...
        return { 
          ...t, // This now includes settledTo from normalizeRow
...
function normalizeRow(row) {
    return {
      ...
      transaction: {
        ...
        settledTo: row.settledTo?.trim() || "" // <--- Added this
      },
    };
  }
